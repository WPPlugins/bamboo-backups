...
/************************************************************************************************************/
	...
	function bamboo_backups_shutdown() {
		...
		if ( 23 < $hour ) {
			$date_of_last_event = strtotime( date( 'M d Y ' ) );
		}
...
/************************************************************************************************************/
	...
	function bamboo_backups_page() {
		...
		if ( isset( $_POST['amboo_backup_settings'] ) ) {
...
/************************************************************************************************************/
	...
	<?php
		foreach ( $backups as $backup ) {
			...
			echo '<label>&nbsp;' . date( 'j F Y, g:i:s a', strtotime( $date ) ),'</label>';
			echo '</th>';
			echo '</tr>';
	    }
	?>
...
/************************************************************************************************************/
	...
	function bamboo_backups_exec() {
		...
		// Before we go any further check that the backup folder contains the .htaccess file to prevent access to the backups via the web for security reasons .
		...
		if ( 'Backup Now' == $_POST['bamboo_backup_now'] ) {
			echo '<div class="updated settings-error" id="setting-error-settings_updated">';
			echo '<p><strong>' . $result . '</strong></p></div>';
		}
